...
//计算两图间psnr值
double aplofIAQ::psnr(Mat referenceImage, Mat sourceImage){
	Mat diffImage;
...
//计算两图间ssim值(OPENCV实现)
double aplofIAQ::cvSsim(Mat referenceImage, Mat sourceImage){
...
//计算两图间ssim值（公式实现）
double aplofIAQ::mySsim(Mat referenceImage, Mat sourceImage){
	const double C1 = 6.5025, C2 = 58.5225;		//C1=(K1*L)^2,C2=(K2*L)^2,C3=C2/2;K1=0.01,K2=0.03,L=255;
	double ssim = 0;
	int count = 1;
	int blockSize = 11;
...
Mat aplofIAQ::calcHashCode(Mat src){
	Mat image;
...
缺点：受均值的影响非常大。
*/
double aplofIAQ::hashDistance(Mat referenceImage, Mat sourceImage){
...
Mat aplofIAQ::calcPHashCode(Mat src){
	Mat image;
...
缺点：速度稍慢。
*/
double aplofIAQ::PhashDistance(Mat referenceImage, Mat sourceImage){
...
Mat aplofIAQ::calcDHashCode(Mat src){
	Mat image;
...
*/
double aplofIAQ::DhashDistance(Mat referenceImage, Mat sourceImage){
...
double aplofIAQ::IQA(Mat referenceImage, Mat sourceImage, int IQAMethod){
...
double aplofIAQ::blockIQA(Mat referenceImage, Mat sourceImage, int size, int IQAMethod){
	if (referenceImage.empty() || sourceImage.empty()){
		return -1;
	}
